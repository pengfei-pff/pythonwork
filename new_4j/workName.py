...
def writer_file():
    csvFile = open(r"F:\TEST3\workName.csv", "w", newline="")  # 创建csv文件
    writer = csv.writer(csvFile)  # 创建写的对象
    writer.writerow(["name:ID(workName-ID)", "data", "link_type", ":LABEL"])  # 写入列的名称
    for i in range(100):
        writer.writerow([(str(i + 1)), addr(), 205, "workName"])
    csvFile.close()
