...
def writer_file():
    csvFile = open("F:\TEST3\Addr.csv", "w",newline="")  # 创建csv文件
    writer = csv.writer(csvFile)  # 创建写的对象
    writer.writerow(["name:ID(Addr-ID)","data","link_type","type",":LABEL"])  # 写入列的名称

    for i in range(1000):
        writer.writerow([(str(i + 1)), ("北京市"+ addr()), 202, [221],"Addr"])
    csvFile.close()
